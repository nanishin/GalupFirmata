...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    ...
    case ANALOG_MAPPING_QUERY:
      ...
      Firmata.write(END_SYSEX);
      break;
    case 0x08: // UltrasoundReport
      Serial.write(START_SYSEX);
      Serial.write(STRING_DATA);
      Serial.println(pollUltrasound(argv[0]));
      Serial.write(END_SYSEX);
      break;
    case 0x18: // NeopixelControl
    {
      // these vars are here for clarity, they'll optimized away by the compiler
      byte pin = argv[0];
      int numpixels = argv[1];
      int color = argv[2];
      int action = argv[3];
      ...
      if (led_list[i].pin > 0) { // Check Neopixel LED Strip
        for(int j=0;j < led_list[i].numpixels; j++){
          ...
          if (led_list[i].action == 1) { // Turn On
            ...
            } else if (led_list[i].color == 1) { // Red Color
                 led_list[i].pixels->setPixelColor(j, led_list[i].pixels->Color(255,0,0)); // Moderately bright green color.
            } else if (led_list[i].color == 2) { // Green Color
                 led_list[i].pixels->setPixelColor(j, led_list[i].pixels->Color(0,255,0)); // Moderately bright green color.
